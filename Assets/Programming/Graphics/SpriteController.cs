...
public class SpriteController : MonoBehaviour
{
    ...
    private int _columns;                        // The number of columns of the texture
    private int _rows;                           // The number of rows of the texture
    ...
    private float _framesPerSecond;            // Frames per second that you want to texture to play at
    private bool _playOnce = false;                  // Enable this if you want the animation to only play one time
    ...
    public bool _newMaterialInstance = true;       // Set this to true if you want to create a new material instance
	...
	public void BeginAnimationOnce (SpriteAnimation sprite)
	{
		if (CurrentSprite != sprite)
			BeginAnimation(sprite);
	}
	
	public bool IsOnFrame(SpriteAnimation sprite, int frame)
	{
		if (CurrentSprite == sprite && frame == _index)
		{
			return true;
		} else {
			return false;
