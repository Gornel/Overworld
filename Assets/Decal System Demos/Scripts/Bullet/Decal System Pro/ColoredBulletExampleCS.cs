			...
			if (Physics.Raycast (l_Ray, out l_RaycastHit, Mathf.Infinity)) {
				...
				if (l_TerrainCollider != null) {
					...
					if (l_Terrain != null) {
						...
						DecalProjector l_DecalProjector = new DecalProjector (l_ProjectorPosition, l_ProjectorRotation, decalProjectorScale, cullingAngle, meshOffset, m_UVRectangleIndex, m_UVRectangleIndex, CurrentColor, 0.0f);
