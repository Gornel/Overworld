...
public class BulletExampleVertexColorUpdateCS : MonoBehaviour {
	...
	private Color[] m_Colors = new Color[3] {Color.red, Color.green, Color.blue};
		...
		if (Input.GetButtonDown ("Fire1")) {
			...
			if (Physics.Raycast (l_Ray, out l_RaycastHit, Mathf.Infinity)) {
				...
				if (l_TerrainCollider != null) {
					...
					if (l_Terrain != null) {
						Color l_Color = m_Colors [m_ColorIndex];
						
							// Create the decal projector with all the required information.
						DecalProjector l_DecalProjector = new DecalProjector (l_ProjectorPosition, l_ProjectorRotation, decalProjectorScale, cullingAngle, meshOffset, m_UVRectangleIndex, m_UVRectangleIndex, l_Color, 0.0f);
